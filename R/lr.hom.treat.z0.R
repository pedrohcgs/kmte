################################################################ This file is used to compute the linear representation of treated individuals with instrument
################################################################ Z=0, for testing the null of homogeneous conditional local average treatment effects, as
################################################################ proposed in the paper 'Nonparametric Tests for Treatment Effect Heterogeneity with Censored
################################################################ data' by Sant'Anna, Pedro H.C. This compute ths linear representation for unconditional term
################################################################ with z=1
...
# fulldata = the entire dataset subdata = sub-sample of treated individuals with instrument Z
# equal to 0 dimx = #of covariates X late = local average treatment effects

lr.hom.treat.z0 <- function(fulldata, subdata, dimx, late) {
    ...
    latekm <- matrix(rep(late[1], n.sub), n.sub)
    if (n.sub > 0) {
        ...
        err <- (subdatakm[, 1] - latekm)/(1 - subdatakm[, (dimfull - 1)])
