...
zclate <- function(out, delta, treat, inst, xvector, xpscore, b, cores = 1, iseed = NULL) {
    ...
    testdist <- (testdist.1 - testdist.0) * fulldata[, 1] * fulldata[, (dim.all - 2)]
    ...
    linrep.control.0 <- lr.lcate.z0(fulldata = fulldata, subdata = data.control.0, dimx = dimx)
    ...
    yest <- (wt + wc) * fulldata[, 1] * fulldata[, (dim.all - 2)]
