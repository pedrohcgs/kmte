...
################################################################ H.C. This file gets the linear represenatation of the conditional term
...
# X ate = average treatment effects

lr.hom.control <- function(fulldata, subdata, dimx, ate) {
    # Dimension of fulldata
    dimfull <- dim(fulldata)[2]
    subdatakm <- subdata
    fulldatakm <- fulldata
    n.sub <- dim(subdatakm)[1]
    nfull <- length(fulldata[, 1])
    atekm <- matrix(rep(ate[1], n.sub), n.sub)
    ...
    err <- (subdatakm[, 1]/(1 - subdatakm[, (dimfull - 1)])) + atekm
