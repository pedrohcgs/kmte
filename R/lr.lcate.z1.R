...
################################################################ testing the null of zero conditional local average treatment effects, as proposed in the paper
...
lr.lcate.z1 <- function(fulldata, subdata, dimx) {
    ...
    if (n.sub > 0) {
        ...
        ################################################################################################### Leading term of the expansion for z=1
        tau.sub <- subdatakm[, 1] * gamma0.sub * subdatakm[, 2]/(subdatakm[, (dimfull - 1)])
