################################################################ This file is used to compute the linear representation of individuals with instrument Z=0, for
################################################################ testing the null of zero conditional local distribution treatment effects, as proposed in the
################################################################ paper 'Nonparametric Tests for Treatment Effect Heterogeneity with Censored data' by Sant'Anna,
################################################################ Pedro H.C.
...
# fulldata = the entire dataset subdata = sub-sample of individuals with instrument Z equal to 0
# dimx = #of covariates X

lr.lcdte.z0 <- function(fulldata, subdata, dimx) {
    ...
    if (n.sub > 0) {
        ...
        tau.sub <- gamma0.sub * subdatakm[, 2]/(1 - subdatakm[, (dimfull - 1)])
