lr.lcdte.z0 <- function(fulldata, subdata, dimx) {
    ...
    if (n.sub > 0) {
        ...
        tau.sub <- gamma0.sub * subdatakm[, 2]/
        ...
        term2.sub <- gamma1.sub * matrix(rep(
          (1 - subdatakm[, 2]), nfull), n.sub, nfull)

        # Now, compute gamma2 and the last term of the expansion
        gamma2.sub <- matrix(rep(
          (1 - subdatakm[, 2])/s1.sub, nfull), n.sub, nfull)
        gamma2.sub <- gamma1.sub * gamma2.sub
        gamma2.sub <- (t(indy1.sub) %*% (gamma2.sub))/n.sub

        # This is the term of the asymptotic expansion free
        # of the estimation effect
